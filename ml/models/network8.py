"""network8.py
______________
An improved version of network.py, implementing the Nestrov's accelerated gradient learning algorithm for a FFNN.
...
class network():
    def __init__(self, sizes, cost=CrossEntropyCost, dropout_rate = 0.5):
        ...
        self.default_velocity_initializer()
        self.cost = cost
    ...
    def default_velocity_initializer(self):
        self.v_w = []
        self.v_b = []
        for i in self.weights:
            self.v_w.append(np.zeros(i.shape))
        for j in self.biases:
            self.v_b.append(np.zeros(j.shape))
    ...
    def backprop(self, x, y, training=True):
        ...
        for b,w in zip(self.biases[:-1], self.weights[:-1]):
            ...
            activation = gelu(z)
            if training:
              mask = (np.random.rand(*activation.shape) < self.dropout_rate) / self.dropout_rate
        ...
        for layer in range(2, self.num_layers):
            ...
            if training:
                delta = delta*self.dropout_mask[-layer+1]
            # Store the gradient
            grad_b[-layer] = delta
            grad_w[-layer] = np.dot(delta,activations[-layer-1].transpose())
        return grad_b, grad_w
    
    def update_mini_batch(self, mini_batch, eta, lmbda, gamma, n):
        total_bias_grad = []
        total_weight_grad = []
        for b in self.biases:
        ...
        for i in range(len(self.weights)):
            self.weights[i] -= gamma*self.v_w[i]
            self.biases[i] -= gamma*self.v_b[i]
        ...
        for i in range(len(self.weights)):
            self.v_w[i] = (
                gamma*self.v_w[i] + (eta/len(mini_batch))*(total_weight_grad[i]) + eta*lmbda*self.weights[i]
                )
            self.v_b[i] = (
                gamma*self.v_b[i] +
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, 
        ...
        for j in range(epochs):
            ...
            for mini_batch in mini_batches:
                self.update_mini_batch(mini_batch, eta, lmbda, gamma, n)
